...
1. ~~자동화 화보 2장~~ ✅ 07-13 완료 — 사용자가 Make 캡처 2장 제공(시트→AI→문서 워크플로우, 프롬프트 조건 설계), `project-3-2/3-3.PNG` 적용 + 캡션·백커버 교체(NEXT ISSUE = 주간 보고서 자동화 예고). ⚠️ P.02 캡처의 프롬프트 내용이 예전 블로그용(네이버 SEO) — 카드 크기에선 거의 안 읽히지만, 주간보고 프롬프트 캡처가 생기면 교체 권장. P.01은 와이드 캡처라 시트·문서 노드가 잘림 — 합성(여백 캔버스) 제안 가능.
2. **이정민 지도 키 origin 등록** — ljm이 https://ljm-ten.vercel.app 에 배포됨(07-13). 네이버 지도 키가 `127.0.0.1:5500`에만 등록돼 있어 location.html 지도가 배포본에서 안 뜸 — 네이버 클라우드 콘솔에서 `ljm-ten.vercel.app` origin 추가 필요(사용자 직접).
3. (보류 유지) Hero 터널 work-17~20 — 이미지는 폴더에 있음, WORKS 배열 4개 추가 + TRAVEL 확장만 하면 됨. 사용자 승인 대기.
4. (보류 유지) Hero B안(중앙 앵커 카피+카운터) 판단.
5. 배포 후 개선(07-13 대부분 완료): ✅ pillar/value WebP 변환(11MB→0.4MB) ✅ 파비콘(favicon.svg + 탭 제목 "추민석 — 반응을 읽는 마케터") ✅ 미사용 파일 삭제(search-demo.webm, ship.png, sea.png, 원본 mp4). 남은 것: works/write/case 대형 PNG 변환(선택), JS 청크 경고(선택), 모바일 실기기 검증, Vercel 중복 정리(miki-ruby=흰 화면, gmcms226-web/portfolio 빈 저장소).

### 07-13 완료 기록

- 웹 케이스스터디 3개 챕터에 배포 링크 칩 추가 — 세종 https://sejong-psi.vercel.app · 이정민 https://ljm-ten.vercel.app · miki https://miki-o1w7.vercel.app (`chapters[].links`, SEJONG/ljm은 Desktop 폴더에서 07-13 Vercel 신규 배포).
- 모바일 항해 영상 간헐 미표시 수정 — play→pause 킥 + poster를 배 보이는 프레임(sea-poster.jpg, 영상 2.5s 추출)으로 교체. sea.png는 이제 미사용.
- 전 구간 검수(데스크톱 8깊이·매거진 12칸·모바일 24깊이) — 깨짐/글자겹침 없음, 콘솔 에러는 파비콘 404뿐.
