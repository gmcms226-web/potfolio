...
- **GitHub**: https://github.com/gmcms226-web/portfolio (비공개) — 집/다른 PC에서는 `git clone` 후 작업
- 재배포: `vercel deploy --prod --yes` (CLI 로그인 필요). GitHub 푸시 자동 배포 연결(`vercel git connect`)은 아직 안 됨 — Vercel 대시보드에서 GitHub 연동하면 푸시만으로 배포됨
...
3. **Vercel–GitHub 자동 배포 연결** — 대시보드에서 GitHub 연동 후 `vercel git connect` 또는 Import.
4. (보류 유지) Hero 터널 work-17~20 — 이미지는 폴더에 있음, WORKS 배열 4개 추가 + TRAVEL 확장만 하면 됨. 사용자 승인 대기.
5. (보류 유지) Hero B안(중앙 앵커 카피+카운터) 판단.
6. 배포 후 개선: 이미지 WebP 최적화(pillar/value/write/voyage/case ~수 MB, 빌드 청크 경고), 미사용 파일 정리(search-demo.webm, ship.png, 원본 mp4), 파비콘 404, 모바일 실기기 검증.
